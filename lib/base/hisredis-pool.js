...
var set = hisredisPool.set = function(key, value){
	...
	var connection = function (err, client) {
		...
		try{
			...
			pool.release(client);
			log.info('REDIS CLIENT {set} ERROR: '+ e);
			dtd.reject(err);
		}
	};
	pool.acquireDb(connection, currDB);
...
var expire = hisredisPool.expire = function(key, expireTime){
	...
	var connection = function (err, client) {
		...
		try{
			...
			pool.release(client);
			log.info('REDIS CLIENT {expire} ERROR: '+ e);
			dtd.reject(err);
		}
	};
	pool.acquireDb(connection, currDB);
...
var keys = hisredisPool.keys = function(pattern){
	var dtd = new $def.Deferred();
	var connection = function (err, client) {
		if(err){
			log.info('REDIS POOL acquireDb ERROR: ' + err);
		...
		try{
			client.keys(pattern, function (err2, reply) {
				pool.release(client);
				if(err2){
					log.info('REDIS {keys} error: '+err2);
					return dtd.reject(err2);
				}
				dtd.resolve(reply);
			});
			...
			pool.release(client);
			log.info('REDIS CLIENT {keys} ERROR: '+ e);
			dtd.reject(e);
		}
	};
	pool.acquireDb(connection, currDB);
