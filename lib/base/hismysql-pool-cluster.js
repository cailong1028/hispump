...
var poolDefaultOptions = {
	...
	// 一定要设置成true, 还有绝对不能再高并发的方法中使用trans, 否则此处设置了true也会造成获取不到链接而移除节点
	// 在设定的数据库连接被占用完之后, 等待, 知道释放连接, 等待超时时间应该就是上面定义的acquireTimeout
...
hismysql.query = function(sql, values, callback, debug){
	var dolog = true;
	...
	var clientConnection = function(err, connection) {
		if(err){
			log.error('POOLCLUSTER ERROR: '+err);
			return callback(err);
		}
		// Use the connection
		...
		//根据value format一次之后，不再将values作为查询条件放入qureyParams， 否则values中的？会再次foramt(cl modify 160114)
		/*if(values){
...
_.extend(Trans.prototype, {
	...
	commit: function(cb){
		...
		var commitCallBack = function(err){
			...
			if(trans.connection){
				try{
					trans.connection.release();
				}catch(e){
					log.error('Fail to release trans connection after commit!');
				}
			}
