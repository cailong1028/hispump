...
var delayRedisBrpopTask = function(delayTime){
	_.delay(redisBrpopTask, delayTime || defaultDelayTime);
};
var brpopDone = function(err, listName, item){
	if(err){
		log('ERROR: task {redisBrpopTask} [brpop] : ' + err);
		return delayRedisBrpopTask();
	}
	if(!item){
		return delayRedisBrpopTask();
	}
	...
	var mysqlDone = function(err, res, fields){
		...
		if(err){
			...
			_.each(upload_his_queues, function(oneQueue){
				if(upload[oneQueue]){
					defs.push(pushToHiss(upload_queues_prefix + oneQueue, item[1]));
				}
			});
...
var pushToHiss = function(queue, data){
...
var redisBrpopTask = function(){
	hisrp.brpop(uploadToMysqlQueue, 0, brpopDone);
};
