...
var beforeServerStart = function () {
	...
	hismpc.poolCluster.getConnection('master', function (err, conn) {
		if (err) {
			log.error('ERROR: Can not connect to DB: ');
			log.error('\t host: [' + firstNode.host + ']');
			log.error('\t port: [' + firstNode.port + ']');
			log.error('\t database: [' + firstNode.database + ']');
			log.error('\t FAIL TO STARTUP APP SERVER');
