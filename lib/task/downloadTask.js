...
var taskSleepTimeWhenNoTriggerDataOrErrorOccur = 10000;
//只查询小于此处设定的失败次数的数据
...
var resolveRowdata = function(oneRow){
	...
	hismpc.query(sql, [tableName, id], function(err, res, fields){
		if(err){
			log('ERROR: there is no row data in table ['+tableName+'], where id = ['+id+']');
			//reject, 告知后续不要删除词条记录
		...
		var pushToDevQueue = function(queueName, dtd){
			...
			hisrp.lpush(queueName, JSON.stringify(downloadDataPackage), function(err, listName, item){
				if(err){
					log('ERROR: lpush to download queue ['+queueName+'] failed: ' + err);
...
var delayDownloadTask = function(delayTime){
	_.delay(downloadTask, delayTime || taskSleepTimeWhenNoTriggerDataOrErrorOccur);
};
var downloadTask = function(){
	...
	log('INFO: task {downloadTask} begin it~s [' + ++taskExeuteCount +'] times invoke');

	//TODO need sort by time
	var sql = 'select * from ?? where ?? < ? order by occurtime asc';
	var queryTriggerDataDone = function(err, res, fields){
		if(err){
			log('ERROR: {downloadTsk} query mysql error: ' + err);
			return delayDownloadTask();
		}
		if(!res || res.length === 0){
		...
		var handleOneRow = function(oneRow){
			...
			var updateTriggerDataWhenError = function(err){
				var updateSql = 'update ?? set ? where ?? = ? and ?? = ?';
				...
				hismpc.query(updateSql, values, function(errUpdate, res, field){
					if(errUpdate){
						log('ERROR: update trigger data error: ' + JSON.stringify(errUpdate).substring(0, 499));
					}
			...
			var handleOneRowDone = function(){
				...
				//非事务和事务两种操作的代码
				var deleteSql = 'delete from ?? where ?? = ? and ?? = ?';
				var values = [triggerDataTableName, triggerDataTableName_table_column, oneRow[triggerDataTableName_table_column], triggerDataTableName_dataid_column, oneRow.DATAID];

				var deleteDone = function(err){
					if(err){
						log('ERROR: DELETE TRIGGER DATA ['+oneRow[triggerDataTableName_table_column]+', '+oneRow.DATAID+'] failed');
				...
				var getTransDone = function(trans){
					...
					trans.commit(function(){
						log('DELETE TRIGGER DATA ['+oneRow[triggerDataTableName_table_column]+', '+oneRow.DATAID+'] SUCCESS');
					});
				};
				var getTransFail = function(err){
					log('ERROR: fail to get mysql transaction when {downloadTask} delete trigger data');
					//记录是按row执行的,各个row之间的数据不影响, 即便上一个row执行失败, 也会continue执行之后的row
					//如果中间某个row dtd使用了reject, 后面的row dtd不会执行
					//即各个def之间不允许dtd.reject
					return dtd.resolve({errMsg: err});
		...
		$def.when.apply($def, defs).done(function(){
			...
			return delayDownloadTask(1000);
		}).fail(function(err){
			log('ERROR: {downloadTask} deferred ERROR: ' + err);
			log('\tBussiness ERROR: if you see here , it meas your CODE is not Correct in Bussiness');
			return delayDownloadTask();
		});
	};
