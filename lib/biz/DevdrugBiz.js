...
var devdrugProto = {
	...
	addDevdrug: function(devDrug){
		...
		//check dev exists
		var sqlCheckDevDrug = 'select * from drugcart where drugid = ? and cartid = ?';
	...
	getDevdrugList: function(queryOptions){
		...
		var from = ' FROM drugcart a LEFT JOIN druginfo b ON a.drugid = b.drugid LEFT JOIN medcart c ON a.cartcode = c.cartcode, fw_dept_dict d ';
		var where = ' WHERE 1 = 1 and c.dept_code = d.dept_code ';

		var values = [];
		var valuesCnt = [];
		if(queryOptions.cartid){
			var cartids = queryOptions.cartid.split(',');
			where += ' and a.cartid in (';
