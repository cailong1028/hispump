...
var appendSheetProto = {
	...
	generateAppendSheet: function(userid, cartcodeList){
		...
		$def.when.apply($def, defs).done(function(){
			...
			var insertPreAppendSheetSql = 'INSERT Pre_AppendSheet (id, CartCode, BoxID, DrugID, DrugBase, DrugStock, AppendCount)' +
				' SELECT replace(uuid(), "-", "") as id, CartCode, BoxID, DrugID, DrugBase, DrugStock, DrugBase - DrugStock AS AppendCount' +
				' FROM drugbox' +
				' WHERE DrugMin > DrugStock AND StorageFlag IN ("0", "1") AND isdeleted = "0"';

			var insertPreAppendSheetValues = [];
			var insertAppendSheetSql =
				'insert appendsheet (id, SheetID, CartCode, DrugID, DrugBase, DrugStock, DrugCount, FactCount, sheetstatus, UserID)'+
				' SELECT replace(uuid(), "-", "") as id, ' +
					//'CONCAT("S", DATE_FORMAT(now(), "%y%m%d%h%i%s")) as SheetID,'+
				' ? as SheetID,'+
				' CartCode, drugID, sum(drugbase) AS drugbase,'+
				' sum(drugstock) AS drugstock, (sum(drugbase) - sum(drugstock)) AS DrugCount, 0, "0", ?'+
				' FROM drugbox'+
				' WHERE DrugMin > DrugStock AND StorageFlag IN ("0", "1") AND isdeleted = "0"';
